...
export default class MessageFilters {
   ...
   */
  static removeSpecialCharacters(message) {
   ...
   */
  static enforceCharacterLimit(message, limit) {
   ...
   */
  static enforceWordCharacterLimit(message, charLimit) {
   ...
   */
  static applyFilters(message, rules) {
    ...
    if (rules.charactersPerWord) {
      filtered = this.enforceWordCharacterLimit(filtered, rules.charactersPerWord);
    }
