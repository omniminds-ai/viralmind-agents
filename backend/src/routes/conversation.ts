  ...
  try {
        ...
        if (type == "tool_call") {
          ...
          try {
            if (toolCall?.name === "computer") {
              ...
              if (!args || !args.action)
                throw new Error(
                  "Missing required fields in tool call arguments"
                );

              const client = await VNCService.ensureValidConnection(
                tournamentPDA
              );
              if (client) {
    ...
    while (true) {
      ...
      try {
        ...
        console.error(
          `Error handling submit (${retryCount}/${maxRetries}):`,
          e
        );
        ...
        if (retryCount >= maxRetries) {
          ...
          res.write(
            `Error: Maximum retry attempts (${maxRetries}) reached. Failing.`
          );
