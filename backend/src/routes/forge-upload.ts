...
// Get upload status
router.get('/status/:uploadId', requireWalletAddress, requireUploadSession, async (req: Request, res: Response) => {
  try {
    // @ts-ignore - Get session from the request object
    const session: UploadSession = req.uploadSession;
...
// Cancel upload
router.delete('/cancel/:uploadId', requireWalletAddress, requireUploadSession, async (req: Request, res: Response) => {
  try {
    // @ts-ignore - Get session from the request object
    const session: UploadSession = req.uploadSession;
...
router.post('/complete/:uploadId', requireWalletAddress, requireUploadSession, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // @ts-ignore - Get walletAddress from the request object
    const address = req.walletAddress;
    console.log(`[UPLOAD] Processing upload for address: ${address}, chunks: ${session.receivedChunks.size}/${session.totalChunks}`);

    // Check if all chunks have been uploaded
    if (session.receivedChunks.size !== session.totalChunks) {
      console.log(`[UPLOAD] Incomplete upload: ${session.receivedChunks.size}/${session.totalChunks} chunks received`);
      const missing = Array.from({ length: session.totalChunks }, (_, i) => i)
    ...
    const sortedChunks = Array.from(session.receivedChunks.values())
    ...
    for (let i = 0; i < sortedChunks.length; i++) {
      ...
      console.log(`[UPLOAD] Writing chunk ${i+1}/${sortedChunks.length} (index: ${chunk.chunkIndex}, size: ${chunk.size} bytes)`);
    ...
    const uploads = await Promise.all(
      requiredFiles.map(async (file) => {
        ...
        console.log(`[UPLOAD] Uploading ${file} (${fileStats.size} bytes) to S3 with key: ${s3Key}`);
