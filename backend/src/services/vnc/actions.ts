...
// todo: figure out what exact type args is
    ...
    switch (action) {
      ...
      case "key":
        ...
        for (const key of keys) {
          if (key === "ctrl" || key === "control") modifiers.ctrl = true;
          ...
          else if (key === "super" || key === "win" || key === "meta")
            ...
            if (key in specialKeys) {
              ...
              console.warn(
                `Unknown key encountered: ${key} - skipping this key`
              );
        ...
        try {
          // Press modifier keys
          if (modifiers.ctrl)
            await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_CTRL, true);
          if (modifiers.alt)
            await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_ALT, true);
          if (modifiers.shift)
            await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_SHIFT, true);
          if (modifiers.super)
            await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_SUPER, true);
          ...
          return `<error>Failed to send key event: ${
          ...
          try {
            if (modifiers.super)
              await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_SUPER, false);
            if (modifiers.shift)
              await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_SHIFT, false);
            if (modifiers.alt)
              await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_ALT, false);
            if (modifiers.ctrl)
              await client.sendKeyEvent(VNC_KEYS.MODIFIERS.LEFT_CTRL, false);
          } catch (err) {
      ...
      case "right_click_drag":
        ...
        // Release at final point
        const rightFinalPoint =
          rightDragTrajectory[rightDragTrajectory.length - 1];
        await client.sendPointerEvent(
          rightFinalPoint.x,
          rightFinalPoint.y,
          false,
      ...
      case "middle_click_drag":
        ...
        // Release at final point
        const middleFinalPoint =
          middleDragTrajectory[middleDragTrajectory.length - 1];
        await client.sendPointerEvent(
          middleFinalPoint.x,
          middleFinalPoint.y,
          false,
