...
<script lang="ts">
    ...
    function isSpecialKey(message: string): boolean {
        const specialKeys = [
            'backspace', 'delete', 'return', 'enter', 'tab', 'space',
            'escape', 'up', 'down', 'left', 'right', 'ctrl', 'alt',
            'shift', 'super', 'meta'
        ];
            ...
            if (event.type === 'keyboard') {
                ...
                if (!isSpecial) {
                    ...
                    if (lastEvent?.type === 'keyboard' && !isSpecialKey(lastEvent.message)) {
                        currentGroup.message = (currentGroup.message + event.message.replace('Key pressed: ', '')).trim();
                    } else {
                        if (currentGroup) {
                            combined.push({
                                ...currentGroup,
                ...
                // Non-keyboard events
                const lastEvent = currentGroup;
                if (lastEvent?.type === event.type && lastEvent?.message === event.message && timeSinceLastEvent < 1000) {
                    repeatCount++;
    ...
    function scrollToEvent(id: string) {
        ...
        const finalScroll = Math.max(minScroll, Math.min(maxScroll, logContainer.scrollTop + scrollTop));
...
<div class="w-[320px] bg-black/95 flex flex-col relative overflow-hidden border-l border-purple-500/30 shadow-[-4px_0_20px_rgba(149,117,205,0.1)]">
    <div class="p-3 bg-black/70 border-b border-purple-500/20 relative z-10">
        <div class="flex items-center gap-2 text-lg font-medium text-purple-400 drop-shadow-[0_0_10px_rgba(149,117,205,0.5)]">
            <Brain size={20} />
            Neural Link
        </div>
        <div class="text-xs text-white/60 mt-0.5 ml-7">
    ...
    <div 
        ...
        class="flex-1 overflow-y-auto p-2 flex flex-col gap-1.5 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-purple-400/30 [&::-webkit-scrollbar-thumb:hover]:bg-purple-400/50 [&::-webkit-scrollbar-thumb]:rounded [mask-image:linear-gradient(to_bottom,transparent,black_20px,black_calc(100%_-_20px),transparent)]"
    >
...
<style>
    ...
    @keyframes glow {
        0% { box-shadow: 0 0 15px rgba(168, 85, 247, 0.2); background: rgba(168, 85, 247, 0.1); }
        50% { box-shadow: 0 0 20px rgba(168, 85, 247, 0.3); background: rgba(168, 85, 247, 0.15); }
        100% { box-shadow: 0 0 15px rgba(168, 85, 247, 0.2); background: rgba(168, 85, 247, 0.1); }
    }
