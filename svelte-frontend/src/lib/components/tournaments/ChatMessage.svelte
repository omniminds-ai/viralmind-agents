<script lang="ts">
  ...
  $: isAgent = firstMessage.role === 'assistant';

  // Process all messages
  $: allContent = messages.map((msg) => {
...
<div class="flex items-start gap-3">
  ...
  <div class="h-8 w-8 flex-shrink-0 overflow-hidden rounded-full">
    {#if isAgent}
      <img
        src={isAgent ? 'https://viralmind.ai' + agentPfp : getJdenticonUrl(firstMessage.address || '')}
        alt="Agent"
  ...
  <div class="min-w-0 flex-1">
    ...
    'bg-stone-800/30' // User messages - vibrant purple, mostly opaque
    } rounded-xl p-3">
      {#each allContent as content, i}
        ...
        {#if content.toolActions.length > 0}
          <div class="space-y-1 text-sm text-gray-400 bg-black/50 rounded-xl p-1 px-2 {i < allContent.length - 1 ? 'mb-2' : ''}">
