...
import { PipelineStage, ProcessedEvent, RawEvent, RawEventsFile, RawSession } from '../../shared/types';
...
export class EventExtractor implements PipelineStage<string, ProcessedEvent[]> {
    ...
    private loadSession(sessionId: string): RawSession {
        const sessionPath = path.join(this.dataDir, 'jailbreak.race_sessions.json');
