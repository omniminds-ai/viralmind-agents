...
export class StructuredDataAugmenter implements PipelineStage<ProcessedEvent[], ProcessedEvent[]> {
    ...
    private async extractTextWithCoordinates(imageBase64: string, timestamp: number): Promise<TextElement[]> {
        if (!this.worker) {
            await this.init();
        }
        ...
        // Log stats
        console.log(`[StructuredDataAugmenter] Extracted ${elements.length} words from frame ${timestamp}`);
...
[{"query":"What buttons appear in the bottom-left corner?","response":{"buttons":[{"text":"Cancel","position":{"x":10,"y":450}},{"text":"Back","position":{"x":80,"y":450}}]}}]`;
    ...
    async process(events: ProcessedEvent[]): Promise<ProcessedEvent[]> {
        ...
        for (const frameEvent of sampledFrames) {
            ...
            try {
                console.log(`[StructuredDataAugmenter] Processing frame ${++completed}/${sampledFrames.length}`);
                
                // Extract text elements with coordinates
                const elements = await this.extractTextWithCoordinates(frameEvent.data.frame, frameEvent.timestamp);
        ...
        console.log(`[StructuredDataAugmenter] Completed structured data analysis. Processed ${completed} frames.\n`);
        await this.cleanup();
        return events;
    }
}
