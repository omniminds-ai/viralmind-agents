...
<div class="min-h-screen bg-black pb-24 text-white">
  <div class="relative flex min-h-screen flex-col items-start justify-center overflow-hidden">
    ...
    <div class="relative z-10 mx-auto w-full max-w-[1400px] px-8">
      ...
      <div class="max-w-2xl mb-12">
        <p class="text-2xl text-gray-400 mb-8">
          Stay tuned while we use the free races to ensure the staked races are perfectly balanced for the best possible experience.
        </p>
        ...
        <button
          class="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-4 px-8 rounded-lg transition-colors flex items-center gap-2 text-lg"
          on:click={() => goto('/gym/free-races')}
        >
          Try Free Races
          <Brain class="w-5 h-5" />
        </button>
    ...
    <!-- Animated Hammer -->
    <div class="absolute inset-0 z-[1] flex items-center justify-center pointer-events-none overflow-visible">
      <div class="hammer-animation">
        <Hammer class="w-[1000px] h-[1000px] text-purple-900 opacity-5" />
      </div>
    </div>
  </div>
  ...
  <div
    class="absolute inset-0 z-[2] transition-transform duration-1000 ease-out opacity-25"
  ...
  ></div>
  <div class="absolute inset-0 z-[3] bg-gradient-to-b from-black via-transparent to-black opacity-25"></div>
</div>
